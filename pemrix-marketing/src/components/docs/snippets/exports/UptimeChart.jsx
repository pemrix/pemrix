...
/*
  Live client component: renders a model's uptime charts by fetching the
  ClickHouse-backed public API at page-render time.
  ...
  Source of truth: https://openrouter.ai/api/frontend/v1/stats/model-uptime-recent?permaslug=...
*/
export const UptimeChart = ({ permaslug }) => {
  ...
  const renderLineSegments = (segments, strokeClass, markerClass) =>
    segments.flatMap((segment, segmentIndex) => {
      const elements = [];
      if (segment.length > 1) {
        elements.push(
          <polyline
            key={`${strokeClass}-${segmentIndex}`}
      ...
      if (segment.length === 1) {
        ...
        elements.push(
          <circle
            key={`${strokeClass}-${segmentIndex}-isolated`}
            cx={x}
            cy={y}
            r="2"
  ...
  const recentDomain = getRecentDomain(buckets, [
    "availability",
    "availabilityWithoutRouting",
  ]);
  const recentSegments = getLineSegments(
    buckets,
    "availability",
    recentDomain.min,
    recentDomain.max,
  ...
  const withoutRoutingSegments = hasAvailabilityWithoutRouting
    ? getLineSegments(
        buckets,
        "availabilityWithoutRouting",
        recentDomain.min,
    ...
    <div className="flex w-full flex-col gap-6 p-4 sm:p-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <svg
          ...
          role="img"
          aria-labelledby={`${chartId}-uptime-hourly-title`}
        >
          ...
          {hourly.map((point, index) => (
            <rect
              key={point.timestamp}
              x={`${index * hourlyBarWidth}%`}
              y="0"
              width={`${Math.max(hourlyBarWidth - 0.3, 0)}%`}
              height="48"
              className={getAvailabilityFillClass(point.availability)}
              fillOpacity={point.availability === null ? 0.2 : 0.95}
            />
          ))}
        </svg>
      ...
      <div className="flex flex-col gap-2">
        ...
        <svg
          ...
          role="img"
          aria-labelledby={`${chartId}-uptime-recent-title`}
        >
        ...
        <div className="text-muted-foreground flex flex-wrap gap-x-4 gap-y-1 text-xs">
          ...
          {hasAvailabilityWithoutRouting && (
            <span>
              <span
                className="mr-1 inline-block size-2 rounded-full bg-yellow-600"
                aria-hidden="true"
              />
              Without Routing
            </span>
      ...
      <div className="sr-only">
        ...
        <ul>
          {buckets.map((bucket) => (
            <li key={`recent-${bucket.timestamp}`}>
              {bucket.timestamp}: availability {formatPercent(bucket.availability)}
              {hasAvailabilityWithoutRouting &&
                `, without routing ${formatPercent(bucket.availabilityWithoutRouting)}`}
            </li>
          ))}
        </ul>
      </div>
