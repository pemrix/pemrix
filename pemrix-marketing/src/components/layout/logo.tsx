...
export default function Logo({
  ...
  const logoSrc =
    typeof isDark === "boolean"
      ? isDark
        ? "/logos/quanvio-dark.svg"
        : "/logos/quanvio-light.svg"
      : undefined;
  ...
  return (
    <Element href={href} className={cn("flex items-center gap-1.75 text-xl font-medium", className)}>
      <span aria-hidden="true" className={cn("relative block size-6.5", iconClassName)}>
        {logoSrc ? (
          <img src={logoSrc} alt="Quanvio" className="size-full object-contain" />
        ) : (
          <>
            <img
              src="/logos/quanvio-light.svg"
              alt="Quanvio"
              className="absolute inset-0 size-full object-contain dark:hidden"
            />
            <img
              src="/logos/quanvio-dark.svg"
              alt="Quanvio"
      ...
      <span className={cn("", wordmarkClassName)}>Quanvio</span>
    </Element>
  );
}
