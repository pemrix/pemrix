...
seoTitle: 'Prompt Caching | Reduce AI Model Costs with OpenRouter'
...
og:image: https://openrouter.ai/dynamic-og?title=Prompt%20Caching&description=Optimize%20AI%20model%20costs%20with%20OpenRouter
og:image:width: 1200
og:image:height: 630
...
<Info>
When using router models like [Auto Router](/guides/routing/routers/auto-router) or [Pareto Router](/guides/routing/routers/pareto-router), sticky routing also reuses the **resolved model** on a best-effort basis when it remains in the current candidate set, not just the provider. The cache hint is ignored when it falls out of that set, so the router may select a different model on a later turn. See [Auto Router — Session Stickiness](/guides/routing/routers/auto-router#session-stickiness) for details.
</Info>
...
<Note>
The [Batch API](/batch-quickstart) does not yet group its generations by `session_id`.
</Note>
