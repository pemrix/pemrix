---
title: Async Tools
description: Run long-running tools in the background or defer them to external systems, while the model keeps working, checks progress, steers, and receives results automatically.
seoTitle: 'Async Tools | OpenRouter SDK'
og:site_name: OpenRouter Documentation
og:title: 'Async Tools - OpenRouter SDK'
og:description: 'Background and deferred tool execution for callModel: pending placeholders, the universal task tool for check-ins and steering, typed cross-process completion, and subagent tools.'
og:image: https://openrouter.ai/dynamic-og?title=Async%20Tools&description=Background%20%26%20Deferred%20Tool%20Execution
og:image:width: 1200
og:image:height: 630
...
Deferred tools require a `StateAccessor` (see [Tool Approval & State](/agent-sdk/call-model/tool-approval-state)) so the paused conversation can be found and resumed from another process.
...
- **[Tools](/agent-sdk/call-model/tools)** - The `tool()` helper, schemas, and tool types
- **[Tool Approval & State](/agent-sdk/call-model/tool-approval-state)** - `StateAccessor` setup for deferred tools and cross-process resume
- **[Streaming](/agent-sdk/call-model/streaming)** - Consuming the full event stream
- **[Lifecycle Hooks](/agent-sdk/call-model/lifecycle-hooks)** - Observing tool execution
