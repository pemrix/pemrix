...
seoTitle: 'OAuth PKCE | Secure Authentication for OpenRouter'
...
og:image: https://openrouter.ai/dynamic-og?pathname=use-cases/oauth-pkce&title=OAuth%20PKCE&description=Secure%20one-click%20authentication%20for%20your%20OpenRouter%20users
og:image:width: 1200
og:image:height: 630
...
<Frame>
<img alt="Alt text" src="/assets/guides/overview/auth/oauth/auth-request.png" />
</Frame>
...
```typescript title="Generate Code Challenge" lines
...
```txt title="Headless (No Callback)" wrap lines
...
<Frame>
<img alt="Alt text" src="/assets/guides/overview/auth/oauth/code-challenge.png" />
</Frame>
...
```typescript title="Extract Code" lines
...
```typescript title="Exchange Code" lines
...
```typescript title="Create Key Links" lines
...
Store the API key securely within the user's browser or in your own database, and use it to [make OpenRouter requests](/api_reference/overview).
