...
description: Set spending limits per workspace with automatic enforcement
seoTitle: 'Workspace Budgets | OpenRouter Spend Controls'
og:site_name: OpenRouter Documentation
og:title: 'Workspace Budgets - Per-Workspace Spend Limits'
og:description: 'Cap workspace spend with daily, weekly, monthly, or lifetime budgets. Requests are blocked automatically when a budget is met.'
og:image: https://openrouter.ai/dynamic-og?title=Workspace%20Budgets&description=Per-workspace%20spend%20limits
og:image:width: 1200
og:image:height: 630
...
<img alt="Workspace budgets section in the Settings dashboard" src="/assets/guides/features/workspaces/workspace-budgets.png" />
...
The **Include BYOK spend** toggle above the budget rows controls whether [BYOK](/guides/overview/auth/byok) inference spend counts toward these limits. It applies to the whole workspace, so it affects every interval at once. See [Including BYOK spend](#including-byok-spend) for the API equivalent.
...
By default, only OpenRouter credit spend counts toward a workspace's budgets. Set `include_byok_in_budgets` to `true` to also count [BYOK](/guides/overview/auth/byok) inference spend — the amount OpenRouter would have charged had the request not used your own provider key:
...
<AccordionGroup>
  ...
  <Accordion title='Do BYOK (Bring Your Own Key) requests count against workspace budgets?'>
    By default, no — workspace budgets apply to OpenRouter-billed spend, so [BYOK](/guides/overview/auth/byok) requests routed with your own provider key don't count.
