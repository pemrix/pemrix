---
title: Render
description: Using OpenRouter with Render Workflows
seoTitle: 'Render Integration | OpenRouter with Render Workflows'
og:site_name: OpenRouter Documentation
og:title: 'Render Integration - OpenRouter with Render Workflows'
og:description: 'Run OpenRouter model calls inside Render Workflows with queuing, retries, and parallel task runs. Covers local development, deployment, and triggering from an app.'
og:image: https://openrouter.ai/dynamic-og?title=Render&description=Using%20OpenRouter%20with%20Render%20Workflows
og:image:width: 1200
og:image:height: 630
...
If a public application will trigger this workflow, also set `APP_URL` to that application's URL. OpenRouter uses `APP_URL` and `OPENROUTER_APP_TITLE` for [app attribution](/app-attribution).

<Note>
Use `openrouter/auto` for OpenRouter's Auto Router. The `openrouter/auto-beta` slug is the early-access track where new routing behavior lands first; see [Auto Router](/guides/routing/routers/auto-router).
</Note>
...
- [OpenRouter Auto Router](/guides/routing/routers/auto-router)
