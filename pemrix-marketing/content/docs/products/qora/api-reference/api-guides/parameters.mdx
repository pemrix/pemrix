---
title: Parameters
seoTitle: 'API Parameters | Configure OpenRouter API Requests'
...
og:image: https://openrouter.ai/dynamic-og?title=API%20Parameters&description=Complete%20guide%20to%20request%20configuration
og:image:width: 1200
og:image:height: 630
...
Please refer to the model’s provider section to confirm which parameters are supported. For detailed guidance, see [managing provider-specific parameters](/guides/routing/provider-selection#requiring-providers-to-support-all-parameters).
...
Tool calling parameter, following OpenAI's tool calling request shape. For non-OpenAI providers, it will be transformed accordingly. Learn more in the [tool calling guide](/guides/features/tool-calling).
