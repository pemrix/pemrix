...
If you have a [provider key](/guides/overview/auth/byok) configured, batches route through it automatically, the same as sync requests: the provider bills you directly for inference and OpenRouter charges only the BYOK fee (see Pricing above). Completed batches report `usage.is_byok: true`.

Google Vertex uses a bucket in your GCP project for provider input and output. The primary setup is frictionless: omit `bucket` and let OpenRouter create a private, location-compatible bucket on the first batch. You can optionally provide an existing bucket instead. See [Google Vertex API keys](/guides/overview/auth/byok#google-vertex-api-keys) for permissions and storage behavior.
...
```text title="Status progression" lines
...
```json title="request_counts" lines
{
  "total": 100,
  "completed": 98,
  "failed": 2
...
Each completed result's `response.body.id` is that request's OpenRouter generation ID (for example `gen-batch-...`). To flag a bad generation, copy that ID and submit it through [Report Feedback](/guides/overview/report-feedback) using the **By generation ID** flow.
...
```json title="Anthropic Messages batch" lines
{
...
```json title="Embeddings batch" lines
{
...
```json title="Embeddings results" lines
[
  {
    "id": "batch_req_emb_1",
    "custom_id": "emb-0001",
