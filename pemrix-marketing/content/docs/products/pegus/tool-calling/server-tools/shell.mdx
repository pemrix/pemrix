---
title: Shell
description: Give any model a sandboxed hosted shell on the Responses and Messages APIs
seoTitle: 'Shell Server Tool | Hosted Shell Execution for Any Model'
og:site_name: OpenRouter Documentation
og:title: 'Shell Server Tool - Hosted Shell Execution for Any Model'
og:description: "Add a hosted shell to any model on OpenRouter's Responses and Messages APIs. A sandbox-backed clone of OpenAI's hosted shell tool: the model runs commands server-side in an isolated container."
og:image: https://openrouter.ai/dynamic-og?title=Shell%20Server%20Tool&description=Hosted%20shell%20execution%20for%20any%20model
og:image:width: 1200
og:image:height: 630
...
The shell server tool is available through the [Responses API](/api_reference/responses/overview) and the [Messages API](/api/api-reference/anthropic-messages/create-a-message). Requesting it on the Chat Completions API returns a `400` error.
...
| `environment` | object | `container_auto` | Execution environment. Use `{ "type": "container_auto" }` for an OpenRouter-managed ephemeral container, or `{ "type": "container_reference", "container_id": "..." }` to reuse an existing container. `local` environments are not supported. See [Containers](/guides/features/server-tools/containers) |
