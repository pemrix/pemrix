...
description: >-
  Automatic code review for Claude Code using hooks
  and OpenRouter — async, non-blocking reviews from a
  second model while you keep working
seoTitle: >-
  Automatic Code Review for Claude Code | OpenRouter
  https://openrouter.ai/docs/cookbook/coding-agents/automatic-code-review
og:site_name: OpenRouter Documentation
og:title: >-
  Automatic Code Review for Claude Code — Async
  Reviews via Hooks + OpenRouter
og:description: >-
  Set up automatic, non-blocking code reviews for
  Claude Code. A stop hook triggers a background
  reviewer agent via OpenRouter — observable,
  customizable, and cost-monitored.
og:image: >-
  https://openrouter.ai/dynamic-og?title=Automatic%20Code%20Review&description=Async%20Reviews%20for%20Claude%20Code%20via%20Hooks%20%2B%20OpenRouter
og:image:width: 1200
og:image:height: 630
...
1. If the JSON contains `"decision": "block"` with a
...
1. If the command exits 0 with no JSON output, Claude
   proceeds normally (no blocking)
1. If the command exits non-zero, it's treated as a
   non-blocking error
...
1. **Blocking.** `codex exec review` can take minutes.
   ...
   cancel.
1. **No filtering.** The hook fires after every
   response, even when Claude just answered a question
   and made no code changes.
1. **Duplicate reviews.** Nothing prevents the hook
...
1. Ask Claude to make a small code change
1. When Claude finishes, the Stop hook fires the
   check — if there are uncommitted changes, Claude
   spawns the review as a background task
1. You can keep working while the review runs
1. When the review completes, Claude reads the output
