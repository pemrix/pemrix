...
description: Use MCP Servers with OpenRouter
seoTitle: 'Using MCP Servers with OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Using MCP Servers with OpenRouter'
og:description: 'Learn how to use MCP Servers with OpenRouter'
og:image: https://openrouter.ai/dynamic-og?title=Using%20MCP%20Servers%20with%20OpenRouter&description=Learn%20how%20to%20use%20MCP%20Servers%20with%20OpenRouter
og:image:width: 1200
og:image:height: 630
...
<Tip>
Building a TypeScript agent? The [`@openrouter/mcp`](/agent-sdk/call-model/mcp-tools) package handles all of this for you — it connects to a remote MCP server, authenticates once, and returns tools you spread directly into `callModel`.
</Tip>
...
First, some setup. To run this you'll need to pip install the packages, and create a `.env` file with OPENAI_API_KEY set. This example also assumes the directory `/Applications` exists.
