---
title: MCP Tools
description: Connect remote Model Context Protocol servers to the Agent SDK with @openrouter/mcp (auth, caching, streaming, resources, and elicitation).
seoTitle: 'MCP Tools | OpenRouter Agent SDK'
og:site_name: OpenRouter Documentation
og:title: 'MCP Tools - OpenRouter Agent SDK'
og:description: >-
  Connect a remote MCP server (Streamable HTTP or SSE) to the Agent SDK.
  Authenticate once, drop the returned tools into callModel, and reuse them
  across cold starts with a serializable cache.
og:image: >-
  https://openrouter.ai/dynamic-og?title=MCP%20Tools&description=Agent%20SDK%20%2B%20Model%20Context%20Protocol
og:image:width: 1200
og:image:height: 630
...
Reach for `@openrouter/mcp` when you want to plug an existing MCP server (Linear, GitHub, an internal tool server) into an Agent SDK loop without hand-rolling the protocol. It complements your own [`tool()`](/agent-sdk/call-model/tools) definitions. MCP tools and locally-defined tools can live in the same `tools` array, and the SDK keeps the two apart with a `source` discriminant so your typed tool results stay typed.

If you only need a hosted MCP server for your editor or coding assistant, see the [OpenRouter MCP server guide](/guides/overview/mcp-server) instead.
...
Wrapped MCP tools behave like [generator tools](/agent-sdk/call-model/tools#generator-tools). Progress notifications the server emits during a tool call surface as `tool.progress` events on the same streams your own tools use:
...
- [Tools](/agent-sdk/call-model/tools), define your own tools with `tool()` and the Zod schema helpers.
- [Streaming](/agent-sdk/call-model/streaming), consume `tool.result` and `tool.progress` events, including the `source` discriminant.
- [OpenRouter MCP server](/guides/overview/mcp-server), the hosted MCP endpoint for your editor or coding assistant.
