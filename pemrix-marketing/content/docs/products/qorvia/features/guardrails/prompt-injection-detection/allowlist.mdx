---
title: Allowlist
description: Exclude known-safe phrases from prompt injection detection
seoTitle: 'Allowlist | OpenRouter Guardrails'
og:site_name: OpenRouter Documentation
og:title: 'Prompt Injection Allowlist - Exclude Safe Phrases from Detection'
og:description: 'Add specific phrases to an allowlist so they are not flagged, redacted, or blocked by the regex-based prompt injection guardrail.'
og:image: https://openrouter.ai/dynamic-og?title=Prompt%20Injection%20Allowlist&description=Exclude%20safe%20phrases%20from%20detection
og:image:width: 1200
og:image:height: 630
...
- **Multiple patterns detected:** click the banner to expand a picker with checkboxes. Select individual patterns or use **Select all**, then click **Add _N_ patterns** to batch-add them. Patterns already on the allowlist are hidden from the picker, and an "N already added" link takes you to [Settings > Privacy](https://openrouter.ai/settings/privacy) to manage them.

<img alt="Multi-select pattern picker in the prompt detail view" src="/assets/guides/features/guardrails/prompt-injection/pi-allow-list-multi-select.png" />
