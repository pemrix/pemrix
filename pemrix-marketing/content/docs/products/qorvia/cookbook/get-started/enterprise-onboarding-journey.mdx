...
description: What to expect from signature to production, and what we need from you at each step
seoTitle: 'Enterprise Onboarding Journey | OpenRouter for Organizations'
og:site_name: OpenRouter Documentation
og:title: 'Enterprise Onboarding Journey - OpenRouter for Organizations'
og:description: 'Typical OpenRouter enterprise onboarding milestones: API keys within 24 hours, first inference within 48 hours, go-live by day 14, and full production pace by day 30.'
og:image: https://openrouter.ai/dynamic-og?title=Enterprise%20Onboarding%20Journey&description=Typical%20enterprise%20onboarding%20milestones
og:image:width: 1200
og:image:height: 630
...
If you want to set up your organization self-serve, follow the [Enterprise Quickstart](/cookbook/get-started/enterprise-quickstart).
...
Once your organization is live, your team can create its own API keys independently through the [Enterprise Quickstart](/cookbook/get-started/enterprise-quickstart), without waiting for the activation call. The call is where your FDE pairs with you on key creation and the rest of the configuration.

### Day 1: activation working session

During the 60-minute activation call, your team will:

1. Create your [workspaces](/guides/features/workspaces)
2. Set your [guardrails](/guides/features/guardrails) and [Zero Data Retention](/guides/features/zdr) policy
3. Set [workspace budgets](/guides/features/workspaces/workspace-budgets)
4. Start SSO domain verification
5. Create a [management key](/guides/overview/auth/management-api-keys) and your first [API key](/api_reference/authentication)
6. Make a first test API call to confirm the key works

Create the management key from [Management API Keys](https://openrouter.ai/settings/management-keys). If your organization uses provider credentials, review [BYOK](/guides/overview/auth/byok) and [API Key Rotation](/cookbook/administration/api-key-rotation).
...
Your team maps workspaces and sets budgets. The FDE walks your team through the structure. Start from your [home dashboard](https://openrouter.ai/workspaces), use [Create Workspace](https://openrouter.ai/workspaces/new), or automate setup with the [Workspaces API](/api/api-reference/workspaces/list-workspaces).

### Day 5: governance

Your team completes [SSO](/guides/features/sso), [SCIM group mappings](/guides/features/scim-mappings), guardrails, and data-retention policy. The FDE provides the DNS record and IdP guide. Review [privacy settings](https://openrouter.ai/settings/privacy), [Data Collection](/guides/privacy/data-collection), [Provider Logging](/guides/privacy/provider-logging), and the [Guardrails API](/api/api-reference/guardrails/list-guardrails) as needed.

### Day 5 to 7: inference strategy and observability session

The second and final required meeting. It covers [provider routing](/guides/routing/provider-selection), [model fallbacks](/guides/routing/model-fallbacks), [prompt caching](/guides/best-practices/prompt-caching), [presets](/guides/features/presets) so your application code names use cases rather than model versions, [Broadcast](/guides/features/broadcast) into your own observability stack, [user attribution](/cookbook/administration/user-tracking), and access to the [Analytics API](/api/api-reference/analytics/query-analytics-data) for your own reporting. Create presets from the [Presets](https://openrouter.ai/workspaces/default/presets) page.

You leave the session with at least one preset in use and traces flowing to your platform.

### Day 7: observability

Your team configures Broadcast with the FDE pairing. Start from [Settings > Observability](https://openrouter.ai/settings/observability), review traces in [Logs](https://openrouter.ai/logs) or your workspace's [Observability settings](https://openrouter.ai/workspaces/default/observability), and compare Broadcast with [Input & Output Logging](/guides/features/input-output-logging).

### Day 10: rollout

Your team rolls out beyond the first team. The FDE supplies SCIM group mappings and developer-ready setup material. Coding-agent teams can use [Ori Harness](/guides/ori/harness) for the rollout.
...
The month-one report covers pace against commitment, spend by team and model, reliability, savings against direct provider access, and recommendations. Use [Usage Accounting](/cookbook/administration/usage-accounting), [Activity Export](/cookbook/administration/activity-export), and the [Activity page](https://openrouter.ai/activity) for reporting.
...
Your team tunes [provider routing](/guides/routing/provider-selection), [prompt caching](/guides/best-practices/prompt-caching), and [presets](/guides/features/presets) during office hours and through the usage and optimization report. Review [Uptime Optimization](/guides/best-practices/uptime-optimization) when tuning reliability and latency.
...
<CardGroup cols={2}>
  <Card title="Enterprise Quickstart" icon="rocket" href="/cookbook/get-started/enterprise-quickstart">
    Organization, workspaces, key management, security controls, presets, and observability, step by step.
  </Card>
  <Card title="Organization Management" icon="building-user" href="/cookbook/administration/organization-management">
    Roles, shared credits, member management, and centralized usage tracking.
  </Card>
</CardGroup>

For application teams expanding beyond the first workload, see the [Quickstart](/cookbook/get-started/quickstart), [Structured Outputs](/guides/features/structured-outputs), [Tool Calling](/guides/features/tool-calling), and [Latency and Performance](/guides/best-practices/latency-and-performance).
