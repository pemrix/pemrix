---
title: Enhance Image Generation with Presets
description: Pair a text model with the image generation server tool so every request gets a refined prompt automatically
seoTitle: 'Enhance Image Generation with Presets | OpenRouter Cookbook'
og:site_name: OpenRouter Documentation
og:title: 'Enhance Image Generation with Presets - OpenRouter Cookbook'
og:description: 'Create a preset that uses a text model to rewrite vague image prompts into detailed ones before generating. One API call, better images.'
og:image: https://openrouter.ai/dynamic-og?title=Enhance%20Image%20Generation%20with%20Presets&description=Better%20images%20through%20automatic%20prompt%20refinement
og:image:width: 1200
og:image:height: 630
...
|---|---|
| <img alt="Bare prompt result: generic watermelon-textured hippo on white background" src="/assets/cookbook/image-generation/bare-prompt-watermelon-hippo.png" /> | <img alt="Enhanced prompt result: photorealistic watermelon hippo sculpture standing in a river at golden hour" src="/assets/cookbook/image-generation/enhanced-prompt-watermelon-hippo.png" /> |
...
The tool's `parameters` object accepts `model` plus all `image_config` fields (quality, aspect_ratio, size, background, output_format, output_compression, moderation). See the [image generation server tool reference](/guides/features/server-tools/image-generation) for the full list and model-specific defaults.
