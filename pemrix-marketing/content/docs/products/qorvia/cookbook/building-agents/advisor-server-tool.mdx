---
title: Build a Token-Efficient Review Agent
description: Use a cheap executor model for routine work and call Advisor only for compact uncertainty checks
seoTitle: 'Build a Token-Efficient Review Agent | OpenRouter Cookbook'
og:site_name: OpenRouter Documentation
og:title: 'Build a Token-Efficient Review Agent with Advisor'
og:description: 'Build an agent workflow that keeps routine reasoning on a cheap model and uses OpenRouter Advisor only after the task is compressed into a short uncertainty packet.'
og:image: https://openrouter.ai/dynamic-og?title=Token-Efficient%20Review%20Agent&description=Use%20Advisor%20for%20compact%20uncertainty%20checks
og:image:width: 1200
og:image:height: 630
...
{/*
  This file lives at content/pages/guides/advisor-server-tool.mdx.
  docs.yml maps it to /docs/cookbook/building-agents/advisor-server-tool.
*/}
