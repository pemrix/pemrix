---
title: Control Costs with the Analytics API
description: Hand your coding agent a management key and the analytics skill, then ask it where your money is going
seoTitle: 'Control Costs with the Analytics API | OpenRouter Cookbook'
og:site_name: OpenRouter Documentation
og:title: 'Control Costs with the OpenRouter Analytics API'
og:description: 'Use your coding agent, the Analytics API, and the openrouter-analytics skill to find wasted spend, drill into the keys causing it, and verify the fix.'
og:image: https://openrouter.ai/dynamic-og?title=Control%20Costs%20with%20Analytics&description=Find%20wasted%20spend%20with%20your%20coding%20agent
og:image:width: 1200
og:image:height: 630
...
import { CopyPromptButton } from "@/components/docs/snippets/exports/CopyPromptButton.jsx";

{/*
  This file lives at content/pages/cookbook/administration/analytics-cost-control.mdx.
  docs.yml maps it to /docs/cookbook/administration/analytics-cost-control.
*/}

**Goal:** Run a cost review on your OpenRouter account using your coding agent, the [Analytics API](/api/api-reference/analytics/query-analytics-data), and the [openrouter-analytics skill](https://github.com/OpenRouterTeam/skills/tree/main/skills/openrouter-analytics).
...
- [Query analytics endpoint](/api/api-reference/analytics/query-analytics-data)
- [Get analytics metadata](/api/api-reference/analytics/get-available-analytics-metrics-and-dimensions)
...
- Read the [Analytics API reference](/api/api-reference/analytics/query-analytics-data) for exact request and response schemas.
