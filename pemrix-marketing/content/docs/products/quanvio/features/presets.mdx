---
title: Presets
description: Manage your LLM configurations
seoTitle: 'Presets | Configuration Management for AI Models'
og:site_name: OpenRouter Documentation
og:title: 'Presets - Configuration Management for AI Models'
og:description: "Learn how to use OpenRouter's presets to manage model configurations, system prompts, and parameters across your applications."
og:image: https://openrouter.ai/dynamic-og?title=Presets&description=Configuration%20management%20for%20AI%20models
og:image:width: 1200
og:image:height: 630
...
- Tools, including [OpenRouter server tools](/guides/features/server-tools/web-search) such as web search, [advisors](/guides/features/server-tools/advisor), and [subagents](/guides/features/server-tools/subagent)
...
1. [Create a preset](https://openrouter.ai/settings/presets/new). For example, select a model and restrict provider routing to just a few providers.
    <Frame>
    <img alt="Creating a new preset" src="/assets/preset-example.png" title="A new preset" />
    </Frame>
...
- **OpenRouter server tools**, for example `openrouter:web_search`, `openrouter:advisor`, and `openrouter:subagent`. The advisor and subagent tools may appear multiple times, one entry per named instance, so a single preset can define a whole roster of named workers (see the [subagent](/guides/features/server-tools/subagent) and [advisor](/guides/features/server-tools/advisor#multiple-advisors) docs).
