---
title: Auto Router
description: Automatically select the best model for your prompt
seoTitle: 'Auto Router | Smart AI Model Selection'
og:site_name: OpenRouter Documentation
og:title: 'Auto Router - Intelligent Model Selection'
og:description: "Automatically select the best AI model for your prompts using OpenRouter's Auto Router, powered by the wisdom of the market."
og:image: https://openrouter.ai/dynamic-og?title=Auto%20Router&description=Intelligent%20AI%20model%20selection
og:image:width: 1200
og:image:height: 630
...
To see which task type your prompt was classified as, opt in to [router metadata](/guides/features/router-metadata) with the `X-OpenRouter-Metadata: enabled` header. The router stage in `openrouter_metadata.pipeline` then carries the tag at `data.task_type`, such as `code:debugging`. The field is absent when classification is unavailable.
...
Sessions also keep requests on the same provider, which works the same way as it does for any other model. See [Provider Sticky Routing](/guides/best-practices/prompt-caching#provider-sticky-routing) for how sessions are identified, how long they last, and how the `x-session-id` header works.
...
To cap what a request may cost, [`provider.max_price`](/guides/routing/provider-selection#max-price) still applies: it filters the endpoints of whichever models the router resolves.
