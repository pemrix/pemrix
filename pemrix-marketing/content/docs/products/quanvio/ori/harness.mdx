---
title: Ori Harness
description: Run your existing agent CLI on OpenRouter with any model, organization guardrails, and one bill
seoTitle: "Ori Harness | Run Your Agent on OpenRouter"
og:site_name: OpenRouter Documentation
og:title: "Ori Harness - Run Your Agent on OpenRouter"
og:description: "Run the agent CLI you already use on OpenRouter. Choose any model, keep your organization guardrails, and get one bill across agents."
og:image: https://openrouter.ai/dynamic-og?title=Ori%20Harness&description=Run%20your%20agent%20on%20OpenRouter
og:image:width: 1200
og:image:height: 630
...
If your agent is connected to the [OpenRouter MCP server](/guides/overview/mcp-server), you can run `/install-ori-harness` in the agent instead of pasting the command above.
...
Set [guardrails](/guides/features/guardrails), allowlists, and per-workspace permissions on your OpenRouter organization, and set [workspace budgets](/guides/features/workspaces/workspace-budgets) there too. OpenRouter applies them to every request, no matter which client sends it, including an agent running through Ori.

## One bill across agents

You pay for Ori traffic through OpenRouter's normal billing and activity reporting. You see it next to your other traffic in [Usage Accounting](/cookbook/administration/usage-accounting) and [Activity Export](/cookbook/administration/activity-export). There's no separate Ori bill.
