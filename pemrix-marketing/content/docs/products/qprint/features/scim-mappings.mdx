...
description: Automatically provision workspace access from your identity provider groups
seoTitle: 'OpenRouter SCIM Group Mappings | Developer Documentation'
og:site_name: OpenRouter Documentation
og:title: 'OpenRouter SCIM Group Mappings'
og:description: 'Map groups provisioned by your identity provider to OpenRouter workspaces so members are automatically granted the right access.'
og:image: https://openrouter.ai/dynamic-og?pathname=guides/features/scim-mappings&title=SCIM%20Group%20Mappings&description=Automatic%20workspace%20provisioning%20from%20your%20IdP
og:image:width: 1200
og:image:height: 630
...
<Info>
  SCIM group mappings are available for organizations on Enterprise plans and require an active [SSO connection](/guides/features/sso).
</Info>
...
- Your organization must have an SSO connection (see [Single Sign-On (SSO)](/guides/features/sso))
...
- **Deactivation or deletion**: when someone is deactivated or deleted in your identity provider, their organization membership and the API keys they created in the organization are deactivated. See [User Deactivation and Deletion](/guides/features/sso#user-deactivation-and-deletion).
...
<AccordionGroup>
  ...
  <Accordion title="Why is my group list empty?">
    Groups appear once your identity provider pushes them via SCIM provisioning. Check your IdP's provisioning configuration, then click **Refresh from IdP**. You also need an active [SSO connection](/guides/features/sso).
  </Accordion>
