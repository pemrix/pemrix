---
title: Service Tiers
description: Control cost and latency tradeoffs with service tier selection
seoTitle: 'Service Tiers | OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Service Tiers - Control Cost and Latency Tradeoffs'
og:description: 'Learn how to use service tiers on OpenRouter to control cost and latency tradeoffs when routing requests to AI providers.'
og:image: https://openrouter.ai/dynamic-og?title=Service%20Tiers&description=Control%20Cost%20and%20Latency%20Tradeoffs
og:image:width: 1200
og:image:height: 630
...
This is the recommended starting point for most traffic. Reach for the `service_tier` parameter below when you need a specific tier regardless of how it compares to the alternatives. See [Nitro](/guides/routing/model-variants/nitro) and [Floor](/guides/routing/model-variants/floor) for the variants in full.
...
```bash title="Anthropic Messages API" lines
...
`service_tier: "fast"` (OpenAI's [Fast mode](https://developers.openai.com/api/docs/guides/fast-mode) rename of priority processing), `service_tier: "priority"`, and Anthropic's native `speed: "fast"` parameter are fully interchangeable on all APIs and providers. Any of the three requests the priority tier (the response reports `priority`), and on Anthropic models with a fast sibling (e.g. [`anthropic/claude-opus-5-fast`](https://openrouter.ai/anthropic/claude-opus-5-fast)) reroutes to the fast sibling (see [Fast Mode](/cookbook/coding-agents/claude-code-integration#fast-mode)).
